...
import { COLORS, ZERO, ONE, TWO, THREE, FOUR, FIVE, SIX, SEVEN, EIGHT, NINE, DRAW_TWO, SKIP, REVERSE, CHOOSE, DRAW_FOUR, BONUS_SPECIALS, WILD_VALUES, FIRE, ICE, LIGHTNING, SHIELD } from './constants.js';
...
export class BalancedDeck {
    ...
    getStats() {
        ...
        for (const card of this.cards) {
            if (card.special) {
                if (BONUS_SPECIALS.includes(card.special)) {
                    stats.bonusCards++;
                }
                else {
                    stats.specialCards++;
    ...
    isBalanced() {
        ...
        return (specialRatio <= this.config.maxSpecialRatio &&
    ...
    addControlledSpecialCards() {
        const targetSpecialCount = Math.floor(this.cards.length * this.config.maxSpecialRatio);
        const currentSpecialCount = this.cards.filter(c => c.special && !BONUS_SPECIALS.includes(c.special)).length;
    ...
    addBonusCards() {
        ...
        const currentBonusCount = this.cards.filter(c => c.special && BONUS_SPECIALS.includes(c.special)).length;
    ...
    balanceCardType(fromType, toType) {
        const fromIndex = this.cards.findIndex(card => {
            if (fromType === 'special')
                return card.special && !BONUS_SPECIALS.includes(card.special);
            if (fromType === 'bonus')
                return card.special && BONUS_SPECIALS.includes(card.special);
            if (fromType === 'wild')
                return card.special && (card.special === CHOOSE || card.special === DRAW_FOUR);
            return false;
        });
        if (fromIndex !== -1) {
    ...
    balanceHand(hand) {
        ...
        if (stats.specialCards > 3) {
            ...
            for (let i = 0; i < hand.length; i++) {
                if (hand[i].special && !BONUS_SPECIALS.includes(hand[i].special)) {
                    const randomColor = COLORS[Math.floor(Math.random() * COLORS.length)];
                    const randomValue = [ONE, TWO, THREE, FOUR, FIVE][Math.floor(Math.random() * 5)];
